...
pub(crate) async fn get_transaction(app_state: &State<api::AppState>,
                                    ...
                                    item_delta: Option<i64>) -> Result<Json<Vec<TransactionItem>>, BadRequest<Json<MessageResponse>>> {
    ...
    match conditional_query_as!(Transaction,
        ...
        Ok(result) => {
            let result: Vec<Transaction> = result;
            Ok(Json(result.into_iter().map(|f| { TransactionItem::from_transaction(f) }).collect()))
        }
...
#[get("/transactions/<id>")]
pub(crate) async fn get_transaction_by_id(app_state: &State<api::AppState>, id: i64) -> Result<Json<TransactionItem>, BadRequest<Json<MessageResponse>>> {
    let storage_system = app_state.get_storage_system();
    let transaction_from_id = Transaction::from(storage_system, id).await;
    match transaction_from_id {
        Ok(transaction_from_id) => {
            match transaction_from_id {
                Some(transaction_from_id) => { Ok(Json(TransactionItem::from_transaction(transaction_from_id))) },
...
pub async fn post_transaction(app_state: &State<api::AppState>, input: Json<TransactionItem>) -> Result<Json<TransactionItem>, BadRequest<Json<MessageResponse>>> {
    let storage_system = app_state.get_storage_system();
    ...
    match Transaction::create(storage_system, input.allocation_id, input.item_delta, input.date).await {
        Ok(result) => { Ok(Json(TransactionItem::from_transaction(result))) }
        Err(err) => { Err(BadRequest(Json(MessageResponse { message: err.to_string() + " from backend" }))) }
    }
}
...
pub async fn patch_transaction(app_state: &State<api::AppState>, id: i64,
                               input: Json<TransactionItem>) -> Result<Json<TransactionItem>, BadRequest<Json<MessageResponse>>> {
    let storage_system = app_state.get_storage_system();
    ...
    match new_value.update(&storage_system).await {
        Ok(res) if res.rows_affected() > 0 => Ok(Json(TransactionItem::from_transaction(new_value))),
...
pub async fn delete_transaction(app_state: &State<api::AppState>, id: i64) -> Result<Json<TransactionItem>, BadRequest<Json<MessageResponse>>> {
    let storage_system = app_state.get_storage_system();
            ...
            match result {
                ...
                Some(result2) => {
                    ...
                    match result2.delete(&storage_system).await {
                        Ok(_) => { Ok(Json(TransactionItem::from_transaction(transaction))) }
                        Err(err) => Err(BadRequest(Json(MessageResponse { message: err.to_string() + " from backend" })))
                    }
                }
            }
...
pub async fn count_transaction_entries(app_state: &State<api::AppState>) -> Result<Json<EntriesCountResponse>, BadRequest<Json<MessageResponse>>> {
    let storage_system = app_state.get_storage_system();
