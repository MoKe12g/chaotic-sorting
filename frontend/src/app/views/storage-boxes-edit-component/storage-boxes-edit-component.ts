...
export class StorageBoxesEditComponent implements OnInit {
  ...
  postStorageBox() {
    this.storageBoxService
      .postStorageBox(this.storageBox)
      .pipe(take(1))
      .subscribe({
        ...
        next: (response) => {
          ...
          this.storageBox = response;
          this.router.navigate(['/storage-box/' + response.id]).then((r) => {
            if (!r) {
  ...
  patchStorageBox() {
    this.storageBoxService
      .patchStorageBox(this.storageBox)
      .pipe(take(1))
      .subscribe({
        error: (e) => alert(e.message),
        next: (response) => {
  ...
  deleteStorageBox() {
    this.storageBoxService
      .deleteStorageBox(this.storageBox.id)
      .pipe(take(1))
      .subscribe({
        error: (e) => alert(e.message),
        next: (response) => {
